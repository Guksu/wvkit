...
- Scrollable panels must set `touch-action: pan-y`. `direction: 'vertical'` does not support vertically scrolling panels: on touch devices the pager cannot change panels at all, even at the end of the panel's scroll. `direction: 'both'` is deprecated: it behaves like horizontal and will be removed in 1.0.
