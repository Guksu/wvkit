...
- **No axis lock and no start slop.** At `scrollTop === 0` every single touch enters `pulling` on `touchstart`, and each `touchmove` with a downward component calls `preventDefault()`. A horizontal carousel or inner scroller inside the root cannot scroll natively while the finger drifts downward at the top of the list, and a plain tap cycles `pulling → resetting → idle` (with `onPull(0, 0)` frames) even without movement. Drive indicators from `distance` / `progress`, not from `state !== 'idle'`.
- **Only the root's `scrollTop` is checked.** A nested scroller that is scrolled down does not stop the pull from starting.
- **Desktop mouse drags also pull.** The pointer path does not call `preventDefault()`, so a downward text-selection drag at the top of the list moves the indicator.
- **`setEnabled(false)` does not cancel a gesture already in progress**; it only blocks new ones.
