...
- 패널 안 이미지에는 `loading="lazy"`를 주세요. `overscan` 창에 한 번도 들어온 적 없는 패널은 문서에 붙어 있지 않아, lazy 이미지는 패널이 처음 보일 때까지 요청되지 않습니다. 그 뒤로 창 밖으로 나간 패널은 문서에 붙은 채 `display: none` 이 됩니다.
