...
- **지연 마운트**: `<ScrollContainer lazy>` 를 주면 각 패널 내용을 그 패널이 처음 렌더 창(화면에 보이는 패널 + 양쪽 `overscan`)에 들어올 때 마운트합니다. 그 뒤로는 계속 마운트해 둡니다. 한 번도 가지 않은 패널은 마운트하지 않으므로, 그 패널의 effect 와 요청도 일어나지 않습니다. 기본값은 `false` 로, 지금처럼 모든 패널 내용을 처음부터 마운트합니다. 한 번 마운트한 내용은 다시 언마운트하지 않으므로 폼 입력과 스크롤 위치 같은 상태가 유지됩니다.
...
- 패널 안 이미지에는 `loading="lazy"`를 주세요. `overscan` 창에 한 번도 들어온 적 없는 패널은 문서에 붙어 있지 않아, lazy 이미지는 패널이 처음 보일 때까지 요청되지 않습니다. 그 뒤로 창 밖으로 나간 패널은 문서에 붙은 채 `display: none` 이 됩니다. 문서에 붙은 lazy 이미지를 언제 불러올지는 브라우저가 정하므로, 다음 패널의 이미지가 넘기기 전에 준비되지 않을 수 있습니다. 미리 불러오려면 `onPanelVisibilityChange` 에서 `src` 를 넣거나, 컴포넌트에 `lazy` 를 쓰세요 ([이미지 뷰어 레시피](/ko/recipes/image-viewer/#사진은-필요하기-직전에-불러온다) 참고).
...
이 규칙으로 만든 전체 화면 사진 뷰어는 [이미지 뷰어 레시피](/ko/recipes/image-viewer/)를 보세요.
...
- **휠·트랙패드** (`wheel: true`, 기본). 페이저 축 방향 휠 제스처 하나에 한 패널이 움직입니다. 델타를 제스처(120ms 안에 이어지는 이벤트) 단위로 누적해 40px을 넘으면 한 번 넘기고, 그 제스처의 나머지는 무시합니다. 그래서 트랙패드 관성이 여러 패널을 건너뛰지 않습니다. 교차 축 성분이 더 큰 휠(가로 페이저에서의 세로 스크롤)은 손대지 않습니다. 그 방향으로 더 스크롤할 수 있는 중첩 스크롤러(`overflow-x: auto` 칩 줄) 위에서도 네이티브에 맡깁니다. 예외가 하나 있습니다. 한 제스처로 패널이 바뀌었으면, 그 제스처의 나머지 페이저 축 휠은 중첩 스크롤러 위에서도 소비합니다. 그래서 새 패널 안의 요소를 스크롤하지 않습니다. 줌 상태에서는 휠이 페이지를 넘기지 않고 패널 안에서 카메라를 움직입니다. `Ctrl` + 휠(트랙패드 핀치가 페이지에 이렇게 옵니다)은 커서를 중심으로 줌합니다. 소비한 휠만 `preventDefault` 하므로 Chromium의 macOS 가로 스와이프 뒤로가기도 막힙니다. Safari에서는 페이지에 `overscroll-behavior-x: none`이 따로 필요합니다.
...
- `zoomTo()`는 새 줌 기준으로 활성 패널 범위 안(양 축)에 카메라를 넣습니다. 줌을 1로 되돌리면 패널 중심에 놓입니다. 스냅 애니메이션 도중이라 카메라가 아직 이전 패널에 더 가까울 때도(예: `onIndexChange` 안) 같습니다.
...
| `onPanelVisibilityChange` | `(index: number, visible: boolean, panel: HTMLElement) => void` | — | 패널이 렌더 창(화면에 보이는 패널 + 양쪽 `overscan`)에 들어오면 `true`, 나가면 `false` 로 호출. 한 번도 들어온 적 없는 패널은 들어올 때까지 알리지 않습니다. 빠진 패널은 알리지 않습니다. 패널 내용을 늦게 채울 때 씁니다. |
...
**`ScrollContainer`** 는 `panels` 를 뺀 모든 [옵션](#옵션)을 prop 으로 받습니다. 그 밖의 `div` 속성(`className` / `class`, `style`, `data-*`, `aria-*`)은 호스트 요소에 붙습니다. Vue 에서는 콜백 옵션 대신 `@index-change`, `@zoom-change`, `@panel-visibility-change` 로 듣습니다. 바뀐 prop 은 훅과 같게 다시 마운트하지 않고 반영합니다. `lazy`(기본 `false`)는 컴포넌트에만 있는 prop 입니다. [지연 마운트](#컴포넌트-scrollcontainer-·-scrollpanel)를 보세요.
...
- **패널 DOM은 절대 언마운트되지 않습니다.** 가상화는 `overscan` 창 밖 패널을 숨길(`display: none`) 뿐이고, 모든 패널이 인스턴스가 살아 있는 동안 메모리에 남습니다. 컴포넌트의 `lazy` 는 첫 마운트만 늦춥니다. 긴 리스트는 패널 안에서 직접 가상화하세요.
...
- **숨겨진 패널은 스크롤 위치가 유지됩니다. 라이브러리가 아니라 브라우저가 지킵니다.** e2e 테스트가 `display: none` 왕복을 Chromium 과 Playwright 의 WebKit(데스크톱·iPhone 에뮬레이션)에서 확인합니다. 실제 iOS 기기에서는 확인하지 않았습니다. `setPanels` 로 뺐다가 다시 넣은 패널은 맨 위에서 시작합니다. Chromium·WebKit·Firefox 모두 문서에서 뗀 요소의 스크롤 위치를 0 으로 되돌리기 때문입니다.
