...
export const t = {
  en: {
    ...
    scrollContainer: {
      ...
      note: 'Drag sideways to switch tabs · scroll up/down inside a tab · pinch to zoom · tap ♡. Panels use touch-action: pan-y, the chip row uses pan-x pan-y (native), the canvas uses none. direction=vertical switches to non-scrolling card panels.',
  ...
  ko: {
    ...
    scrollContainer: {
      ...
      note: '옆으로 끌면 탭 전환 · 탭 안에서 위아래 스크롤 · 핀치 줌 · ♡ 탭. 패널은 touch-action: pan-y, 칩 줄은 pan-x pan-y(네이티브), 캔버스는 none. direction=vertical이면 스크롤되지 않는 카드 패널로 바뀝니다.',
